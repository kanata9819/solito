...
impl TerminalState {
    ...
    /// Take replies generated by terminal queries, to send back to the PTY.
...
mod tests {
    ...
    #[test]
    fn terminal_queries_reply_at_the_query_position_across_read_boundaries() {
        let mut state = terminal(8, 3);
        assert!(state.take_responses().is_empty());
        state.apply_terminal_output(b"history\r\nA\r\nB\r\nCD\x1b[");
        assert!(state.take_responses().is_empty());
        state.apply_terminal_output(b"6n\rZ\x1b[6n\x1b[5n");
        assert_eq!(state.take_responses(), b"\x1b[3;3R\x1b[3;2R\x1b[0n");
        assert!(state.take_responses().is_empty());
        state.apply_terminal_output(b"\x1b[2;3r\x1b[?6h\x1b[1;4H\x1b[6n");
        assert_eq!(state.take_responses(), b"\x1b[1;4R");
    }
